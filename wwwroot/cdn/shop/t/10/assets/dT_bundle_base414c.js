...
Line 319:2 Transforming const to the configured target environment ("es5") is not supported yet
Line 341:2 Transforming const to the configured target environment ("es5") is not supported yet
Line 361:0 Transforming const to the configured target environment ("es5") is not supported yet
Line 363:2 Transforming const to the configured target environment ("es5") is not supported yet
Line 365:2 Transforming let to the configured target environment ("es5") is not supported yet
...
$(document).on("submit", "#bundleAddToCartFrmId", function(event) {
  ...
  //const cartContent = document.querySelector('cart-notification') || document.querySelector('cart-drawer');
  ...
  const doBundleAddToCart = function() {
    ...
    Shopify.moveAlong = function() {
      if (Shopify.queue.length) {
        console.log('one');
        dT_AddToCartOther(Shopify.queue);
      }
    };
...
const dT_AddToCartOther = function(products) {
   
  const cartContent = document.querySelector('cart-notification') || document.querySelector('cart-drawer');
  
  let formData = {
    items: products,
    sections: cartContent.getSectionsToRender().map((section) => section.id),
  ...
  .then((response) => {
    cartContent.classList.contains('is-empty') && cartContent.classList.remove('is-empty');
    cartContent.renderContents(response);
  })
  .catch((error) => {
